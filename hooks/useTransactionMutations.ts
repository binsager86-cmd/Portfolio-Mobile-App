...
const TXN_DEPENDENT_QUERY_KEY_SET = new Set<string>(TXN_DEPENDENT_QUERY_KEYS);
...
async function invalidateTransactionCaches(
  ...
  await queryClient.invalidateQueries({
    predicate: (query: { queryKey: readonly unknown[] }) => {
      const [queryRoot] = query.queryKey;
      return typeof queryRoot === "string" && TXN_DEPENDENT_QUERY_KEY_SET.has(queryRoot);
    },
  });
}
