...
def calculate_future_value(pv: float, years: int, freq: int, rate: float, pmt: float) -> tuple:
    """Calculate Future Portfolio Value using beginning-of-period contributions."""
    total_periods = years * freq
    periodic_rate = (rate / 100) / freq
    
    if periodic_rate > 0:
        fv_pv = pv * ((1 + periodic_rate) ** total_periods)
        fv_pmt = pmt * (((1 + periodic_rate) ** total_periods - 1) / periodic_rate) * (1 + periodic_rate)
        result = fv_pv + fv_pmt
    else:
...
def calculate_required_yield(pv: float, years: int, freq: int, target_fv: float, pmt: float) -> tuple:
    ...
    max_supported_fv, *_ = calculate_future_value(pv, years, freq, high_rate * 100, pmt)
    if target_fv > max_supported_fv:
        raise HTTPException(
            status_code=400,
            detail="Target future value requires an annual yield above the supported 100% maximum",
        )
...
def calculate_required_contribution(pv: float, years: int, freq: int, rate: float, target_fv: float) -> tuple:
    """Calculate Required Periodic Contribution for beginning-of-period payments."""
    total_periods = years * freq
    periodic_rate = (rate / 100) / freq
    ...
    if periodic_rate > 0:
        ...
        annuity_factor = (((1 + periodic_rate) ** total_periods - 1) / periodic_rate) * (1 + periodic_rate)
...
@app.get("/cron/{action}")
async def cron_endpoint_get(action: str):
    """Cron actions are state-changing; use POST with X-Cron-Key."""
    raise HTTPException(status_code=405, detail="Use POST /cron/{action} with X-Cron-Key header")


@app.post("/cron/{action}")
async def cron_endpoint(action: str, x_cron_key: str = Header(default="", alias="X-Cron-Key")):
    """
    Cron endpoint for automated daily price updates and snapshots.
    
    Usage:
        POST /cron/daily_update with X-Cron-Key: YOUR_SECRET_KEY
        POST /cron/recalculate with X-Cron-Key: YOUR_SECRET_KEY
    """
    import os
    ...
    try:
        ...
        if action in {"update_prices", "snapshot"}:
            raise HTTPException(
                status_code=501,
                detail=f"Cron action '{action}' is not implemented as a separate job; use daily_update",
            )
        ...
        logger.exception("Cron import failed for action %s", action)
        raise HTTPException(status_code=500, detail="Cron dependency unavailable")
    except Exception as e:
        logger.exception("Cron action failed: %s", action)
        raise HTTPException(status_code=500, detail="Cron execution failed")
