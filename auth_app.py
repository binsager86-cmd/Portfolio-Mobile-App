...
def is_password_strong(password: str) -> bool:
    """Minimum password policy shared by registration and reset flows."""
...
def send_otp_email(to_email: str, otp: str) -> bool:
    """Send OTP email. Returns False when SMTP is unavailable or fails."""
    ...
    # =============================
        ...
        with st.form("verify_otp_form"):
            ...
            if btn_verify:
                if new_pass_1 != new_pass_2:
                ...
                elif not is_password_strong(new_pass_1):
                    ...
                    db_execute(
                        cur,
                        "SELECT otp, attempts FROM password_resets WHERE email=? AND expires_at > ? ORDER BY created_at DESC LIMIT 1",
                        (target_email, now),
                    )
                    row = cur.fetchone()
                    ...
                    if row and int(row[1] or 0) >= MAX_OTP_VERIFY_ATTEMPTS:
                        ...
                        if row:
                            db_execute(cur, "UPDATE password_resets SET attempts = attempts + 1 WHERE email=? AND expires_at > ?", (target_email, now))
                            conn.commit()
                        conn.close()
        ...
        with st.form("register_form"):
            ...
            if submit_reg:
                ...
                if reg_pass != confirm_pass:
                    st.error("Passwords do not match")
                elif not is_password_strong(reg_pass):
                    st.warning(f"Password must be at least {PASSWORD_MIN_LENGTH} characters")
