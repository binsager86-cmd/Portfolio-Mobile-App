...
(t-90, t-60, t-30, t-14, t-7, t-3, t-1, t-0) so the ML pipeline can learn
...
from forensics.move_detector import MoveEvent
...
# Definitions of "signals" — atomic conditions that either fire or don't
# Each takes a row of indicator values and returns True/False.
# The forensic recorder finds the earliest day each signal fired before
# the acceleration day, so we know which signals are reliable early warnings.
...
def record_forensic_snapshot(
    ...
    # Locate acceleration day in the indicators DataFrame
    ...
    # Snapshot indicator values at each lookback day
    snapshots = {}
    for lb in lookbacks:
        pos = accel_pos - lb
        if pos < 0:
        ...
        # Convert numpy to python types for cleanliness
        snap_clean = {}
    ...
    # Build signal sequence: for each signal, find the EARLIEST day before
    # acceleration where it first started firing (and stayed firing)
    sequence = []
    for signal_name, signal_fn in SIGNAL_DEFS.items():
        first_fire_pos = None
        # Walk backwards from accel day up to 90 days
            ...
            try:
                if signal_fn(indicators_df.iloc[pos]):
                    ...
                    if first_fire_pos is not None:
                        # was firing, now off — keep the most recent continuous start
                        break
            except Exception:
                continue
        if first_fire_pos is not None:
    ...
    # Sort by days_before descending (earliest signals first)
