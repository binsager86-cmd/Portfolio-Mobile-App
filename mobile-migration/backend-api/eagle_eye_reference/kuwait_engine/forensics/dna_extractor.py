...
For each stock, take all its forensic event snapshots and distill them into the
stock's *personality* — its behavioral fingerprint that the live engine will
compare current conditions against.

This is the answer to: "What does THIS specific stock look like before it moves?"
"""
...
import numpy as np
import pandas as pd
from forensics.recorder import ForensicSnapshot
...
class SignalReliability:
    ...
    reliability_pct: float           # % of real moves where this signal preceded acceleration
    false_positive_rate: float       # % of fakeouts where this signal also fired
    discriminative_power: float      # reliability - fpr (higher = better)
...
def _aggregate_signal_reliability(
    ...
    """For each signal, compute how reliably it preceded real moves vs fakeouts."""
...
def _classify_personality(dna_inputs: Dict[str, Any]) -> str:
    """Tag the stock's overall personality based on aggregate behavior."""
    ...
    if avg_consol > 40 and avg_duration > 60:
        return "slow_builder"           # NBK-like — long quiet then sustained moves
    if avg_consol < 15 and avg_duration < 30:
        return "volatile_burst"         # quick rips, short-lived
    if avg_magnitude > 40 and avg_duration > 45:
        return "high_amplitude_trender"  # big moves, sustained
    if avg_magnitude < 20:
        return "range_grinder"          # small moves, mean-reverting
...
def extract_dna(
    ...
    if len(snapshots) < 3:
        return None  # not enough events to learn from
    ...
    for threshold in thresholds:
        ...
        # Earliest reliable signals: top by discriminative_power AND lead days >= 14
        early = sorted(
            [r for r in reliability.values() if r.avg_lead_days >= 14],
            key=lambda r: -r.discriminative_power,
        )[:5]
        # Confirmation: top by discriminative power with lead days <= 7
        confirmation = sorted(
    ...
    # Aggregate-overall signal reliability
    ...
    # Fakeout signatures: signals that fire OFTEN in fakeouts vs rarely in real moves
    fakeout_sigs = []
