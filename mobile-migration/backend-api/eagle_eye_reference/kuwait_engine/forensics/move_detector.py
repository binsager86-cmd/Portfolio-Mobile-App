...
For every stock, scans its full history and identifies:
  - Every significant upward move at multiple thresholds (10%, 15%, 25%, 50%, 100%)
  - Every significant downward move (for symmetric learning)
  - The exact "acceleration day" each move began
  - The pre-move "quiet period" preceding each move
  - Fake-outs (moves that started but failed within 5 days) — the control group

Each detected move becomes a forensic event the ML pipeline learns from.
"""
...
class MoveEvent:
    ...
    threshold_pct: float            # which threshold tier triggered this event
    start_date: date                # the local low (for UP) or high (for DOWN)
    acceleration_date: date         # the day the move broke out (volume + momentum confirm)
    peak_date: date                 # the local peak/trough reached
    start_price: float
    acceleration_price: float
    peak_price: float
    gain_pct: float                 # signed move from start to peak
    duration_days: int
    days_consolidating_before: int  # how long was the quiet period before acceleration
    pre_move_volatility_pct: float  # 30-day ATR pct at start
    is_fakeout: bool = False        # True if move reversed within 5 days of acceleration
    failed_at_pct: Optional[float] = None  # for fakeouts, how far it got before failing
...
def detect_moves(
    ...
    """
    Detect every move in this stock's history at every threshold.

    Algorithm:
      1. For each day, look forward up to max_lookahead_days
      2. Find the highest high reached in that window (for upward moves)
      3. If gain from this day's low to that high >= threshold, candidate move
      4. Filter to ensure this is a *local* low (no lower low in prior 10 bars)
      5. Find the acceleration day: first day after start where conditions confirm
         (volume > 1.5x avg AND close > prior 20-day high)
      6. Categorize: real move vs fakeout
    """
    if len(ohlcv) < 100:
        return []
    ...
    used_starts = set()  # avoid duplicate events from overlapping windows

    for threshold in sorted(thresholds_pct):
        for i in range(20, len(ohlcv) - min_duration_days):

            # Must be a local low (lower than prior 10 lows)
            if i >= 10 and lows[i] > np.min(lows[i-10:i]):
                continue

            # Skip if we already created a move from this start
            ...
            # Find the highest high reached in window
            ...
            # Find acceleration day: first day after start with volume > 1.5x avg
            # AND close above 20-day high (real breakout confirmation)
            accel_idx = None
            ...
            if accel_idx is None:
                # Fallback: first day where close rose >2% on above-avg volume
                for j in range(i + 1, min(i + 15, peak_idx + 1)):
                    if (closes[j] / closes[j-1] - 1 > 0.02 and
                        ...
                        break
            if accel_idx is None:
                continue  # no clean acceleration — skip

            # Check for fakeout: did price drop > 50% of move within 5 days of accel?
            is_fakeout = False
            failed_at = None
            ...
            # Days consolidating before acceleration: count of bars within 5% range before start
            ...
            events.append(event)
            # Mark this start as used for this and all lower thresholds
...
def detect_fakeouts(
    ...
    """
    Find breakouts that LOOKED real but failed quickly.
    These are the control group for the ML model — what NOT to trust.
    """
    fakeouts = []
    if len(ohlcv) < 50:
        return fakeouts
