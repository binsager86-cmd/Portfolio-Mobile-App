...
class EngineConfig:
    ...
    # Each threshold creates a separate event set the ML pipeline learns from
    ...
    LIQUIDITY_CAP_PCT_OF_DAILY_TURNOVER: float = 10.0  # never own >10% of avg daily turnover
    CIRCUIT_BREAKER_DRAWDOWN_PCT: float = 25.0         # halt new entries on 25% drawdown
    CONFIRMATION_MODAL_THRESHOLD_PCT: float = 30.0     # ask user above this size
    ...
    MIN_DAILY_TURNOVER_KWD: float = 25_000.0  # liquidity gate for tradeable universe
    KUWAIT_TIMEZONE: str = "Asia/Kuwait"
    TRADING_DAYS: tuple = ("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday")

    # --- Regime detection ---
    REGIME_INDEX_TICKER: str = "PMI"  # Premier Market Index
    REGIME_EMA_PERIOD: int = 50  # 50-week EMA on weekly bars
    BRENT_TICKER: str = "BRENT"  # oil overlay
    BRENT_LOOKBACK_DAYS: int = 30

    # --- Retraining cadence ---
    RETRAIN_DAY: str = "Friday"  # post-close weekly retrain
    BEHAVIORAL_DNA_REFRESH_DAYS: int = 7


# Indicator category mapping — every indicator gets a category for confluence scoring
...
# Stage colors for UI (avoiding standard green/red — readable in dark/light)
STAGE_COLORS = {
    "DORMANT":               "#5F5E5A",  # neutral gray
    "STEALTH_ACCUMULATION":  "#185FA5",  # deep blue
    "EARLY_BREAKOUT":        "#0F6E56",  # teal
    "MARKUP_TRENDING":       "#3B6D11",  # green
    "ACCELERATION_CLIMAX":   "#BA7517",  # amber (caution)
    "DISTRIBUTION_TOPPING":  "#993C1D",  # coral
    "MARKDOWN_DECLINE":      "#A32D2D",  # red
    "CAPITULATION_EXHAUSTION":"#534AB7",  # purple (extreme oversold opportunity)
}
