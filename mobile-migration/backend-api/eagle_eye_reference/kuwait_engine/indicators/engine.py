...
def _rolling_pctile(series: pd.Series, window: int) -> pd.Series:
    ...
    def _pctile_of_last(x):
        # x is a numpy array; rank the last value within the window
        last = x[-1]
...
def rsi_divergence(df: pd.DataFrame, lookback: int = 28) -> pd.Series:
    ...
    for i in range(lookback, len(df)):
        ...
        p_min_idx, p_max_idx = window_price.idxmin(), window_price.idxmax()
        r_min_idx, r_max_idx = window_rsi.idxmin(), window_rsi.idxmax()
...
def bb_squeeze(df: pd.DataFrame, period: int = 20, lookback: int = 252) -> pd.Series:
    ...
    return (bw_pct < 20).astype(int)  # 1 if in bottom quintile of bandwidth
...
def volume_profile(df: pd.DataFrame, lookback: int = 90, buckets: int = 50) -> Dict[str, float]:
    ...
    for _, row in recent.iterrows():
        # Distribute the bar's volume proportionally across its range
...
def accumulation_score(df: pd.DataFrame) -> pd.Series:
    ...
    Higher = more institutional buying signature.
    """
    if len(df) < 60:
...
def wyckoff_phase(df: pd.DataFrame, lookback: int = 60) -> pd.Series:
    ...
    A=stopping action, B=cause-building, C=test/spring, D=markup, E=expansion."""
    if len(df) < lookback:
...
def compute_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    as columns. This is the canonical 'indicator snapshot' source of truth
    consumed by the forensics engine, ML pipeline, and live rating engine.
    """
    if len(df) < 50:
